...
function BirthdayTitle({ name, startDateTime }) {
    ...
    return (
        <div className="birthday-container">
            ...
            <CountdownTimer
                targetDate={playtimeEnd}
                color="#EB4781"
                timerName="Play Time"
                endText={`Kids can no longer play`}
                earlyText={`Not Started please wait for party host`}
                earlyCutoff={60}
            />
            <CountdownTimer
                targetDate={partyTimeEnd}
                color="#2CF6B3"
                timerName="Party Room"
                endText={`Party Time Over Head to desk to pay`}
                earlyText={`Set up time`}
                earlyCutoff={120}
            />
        </div>
    );
